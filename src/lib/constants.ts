...
export const STOCK_STATUS_OPTIONS = ["现货", "期货"] as const;

// 新顺序: 产品名称 → 供应商 → 期货/现货 → 上架时间 → 产品规格 → 规格SKU → OE码 → 拿货价格(元) → 销售价格(元) → 适配车型 → 产品链接 → 产品重量 → 产品尺寸 → 包装重量 → 包装尺寸 → 装箱数 → 备注
...
export const DEFAULT_COLUMNS: ColumnName[] = [
  "产品名称", "供应商", "期货/现货", "上架时间", "产品规格", "拿货价格(元)",
  "销售价格(元)", "适配车型", "产品链接"
];

// 列预设方案
export const COLUMN_PRESETS: Record<string, ColumnName[]> = {
  "默认视图": ["产品名称", "供应商", "期货/现货", "上架时间", "产品规格", "拿货价格(元)", "销售价格(元)", "适配车型", "产品链接"],
  "采购视图": ["产品名称", "供应商", "期货/现货", "上架时间", "规格SKU", "拿货价格(元)", "产品链接", "产品重量", "包装尺寸", "装箱数"],
  "销售视图": ["产品名称", "期货/现货", "上架时间", "产品规格", "销售价格(元)", "适配车型", "OE码"],
  "仓库视图": ["产品名称", "期货/现货", "产品尺寸", "包装尺寸", "包装重量", "装箱数"],
};
