...
export function Toolbar({
  ...
  async function handleFileImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const embeddedImages: Map<string, string> = new Map();
      try {
        const sheetObj = sheet as Record<string, unknown>;
        const images = sheetObj["!images"] as
          | Array<{
              base64: string;
              ref: string;
        ...
        if (images && Array.isArray(images)) {
          for (const img of images) {
            if (img.ref) {
              try {
                const cell = XLSX.utils.decode_cell(img.ref);
                embeddedImages.set(`${cell.r}_${cell.c}`, img.base64);
              } catch {}
            }
          }
        }
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const urlColValue =
          ...
          (row["产品图片"] as string) ||
          "";
        if (typeof urlColValue === "string" && urlColValue.trim()) {
          const splitUrls = urlColValue
            .split(/[\n,;，；]+/)
            .map((u) => u.trim())
            .filter(Boolean);
          urls.push(...splitUrls);
        ...
        for (const [key, base64] of embeddedImages) {
        ...
        if (urls.length > 0) {
          imageUrlMap[String(excelRowNum)] = urls;
        }
      }

      const formData = new FormData();
      formData.append("file", file);
      formData.append("imageUrlMap", JSON.stringify(imageUrlMap));
      ...
      // 改进错误处理：先尝试解析 JSON，失败则显示 HTTP 状态
      let result: Record<string, unknown> = {};
      try {
        result = await res.json();
      } catch {
        toast.error(`导入失败: HTTP ${res.status} ${res.statusText}`);
        return;
      }

      if (!res.ok || result.error) {
        // 显示后端返回的详细错误信息
        const errorMsg = String(result.error || "导入失败");
        // 如果错误信息很长，用 description 显示完整内容
        if (errorMsg.length > 100) {
          toast.error("导入失败", { description: errorMsg, duration: 8000 });
        } else {
      ...
      if (data?.imageErrors && data.imageErrors.length > 0) {
        toast.warning(`${data.imageErrors.length} 个图片处理失败`, {
    ...
    <div className="flex gap-2 items-center flex-wrap">
      ...
      <div className="relative">
        ...
        >
          ...
          {supplierFilter
            ? suppliers.find((s) => String(s.id) === supplierFilter)?.name || "供应商"
            : "全部供应商"}
          {supplierFilter && (
            <span
        ...
        {supplierSelectorOpen && (
          <>
            <div
              ...
              onClick={() => setSupplierSelectorOpen(false)}
            />
              ...
              <div className="p-2 border-b border-gray-100">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // 可以通过 filteredSuppliers state 来实现搜索，但目前简单处理
                  }}
                />
              </div>
              <div className="py-1">
        ...
        {colSelectorOpen && (
          <div className="absolute top-full right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg p-3 z-50 w-[280px] max-h-[400px] overflow-y-auto smooth-appear">
            <div className="flex gap-1 flex-wrap mb-2">
              {Object.keys(COLUMN_PRESETS).map((name) => (
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="text-xs h-6 border-gray-200 text-gray-600 hover:border-emerald-300"
            ...
            {GROUP_COLUMNS.map((col) => {
              const isViewerRestricted = role === "viewer" && ["供应商", "产品链接", "备注"].includes(col);
              if (isViewerRestricted) return null;
              return (
                <label
                  key={col}
                  className="flex items-center gap-1.5 py-0.5 text-xs cursor-pointer hover:bg-emerald-50/50 rounded px-1"
                >
                  <Checkbox
            ...
            <div className="text-xs font-medium text-emerald-700 mb-1 mt-2">
              规格信息
            </div>
            {SPEC_COLUMNS.map((col) => {
              const isViewerRestricted = role === "viewer" && col === "拿货价格(元)";
              if (isViewerRestricted) return null;
              return (
                <label
                  key={col}
      ...
      {role !== "viewer" && (
        <>
          ...
          <Button
            ...
            className="border-gray-200 text-gray-600 hover:bg-gray-50 premium-btn"
          >
          ...
          <Button
            ...
            className="border-gray-200 text-gray-600 hover:bg-gray-50"
          >
            <Download className="w-3.5 h-3.5 mr-1" /> 导出Excel
          </Button>
            ...
            <>
              ...
              >
                ...
                {bulkDeleting
                  ...
                  ? `删除选中(${selectedProductIds.length})`
                  : "批量删除"}
              </Button>
